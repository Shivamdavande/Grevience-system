...
import { Send, Search, LogOut, LayoutDashboard, HelpCircle, FileText, CheckCircle, Home, ShieldCheck, ExternalLink, Mail, Phone, MapPin, PlusCircle, Globe, Menu, X as CloseIcon } from 'lucide-react';
    ...
    <header className="sticky-header">
      <div className="container header-inner">
        <div className="logo-section" onClick={() => handleViewChange('home')} style={{ cursor: 'pointer' }}>
          <div className="header-logo-icon" style={{ background: 'var(--gov-navy)', color: 'white', padding: '0.6rem', borderRadius: '4px', display: 'flex' }}>
            <ShieldCheck size={28} />
          </div>
          <div className="header-brand">
            <h1 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--gov-navy)', margin: 0, lineHeight: 1 }}>{t('nav.brand') || 'JANSAHAYAK'}</h1>
            <p className="hidden-mobile" style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--gov-text-muted)', margin: 0, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{t('common.govOfIndia')}</p>
          </div>
        </div>
        ...
        <div className="header-actions" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div className="lang-selector hidden-mobile" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: '#f1f5f9', padding: '0.4rem 0.8rem', borderRadius: '20px', border: '1px solid #e2e8f0' }}>
            ...
            >
              ...
              {/* ... other options omitted for brevity but should be kept ... */}
        ...
        <div className={`mobile-menu-overlay ${isMenuOpen ? 'open' : ''}`}>
          <div className="mobile-nav-header" style={{ marginBottom: '1rem' }}>
            <h3 style={{ color: 'var(--gov-navy)', fontSize: '1rem', fontWeight: 800 }}>MENU</h3>
          </div>
          ...
          <div style={{ marginTop: 'auto', paddingTop: '2rem', borderTop: '1px solid var(--gov-border)' }}>
            <div className="lang-selector" style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Globe size={18} color="var(--gov-navy)" />
              <select 
                onChange={(e) => changeLanguage(e.target.value)} 
                value={i18n.language}
                style={{ flex: 1, padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--gov-border)', fontWeight: 700 }}
              >
                <option value="en">ENGLISH</option>
                <option value="hi">हिन्दी</option>
                <option value="mr">मराठी</option>
            ...
            {(isAuthenticated || isDeptAuthenticated) ? (
              <button onClick={handleLogout} className="btn-gov-secondary" style={{ width: '100%', justifyContent: 'center' }}>{t('nav.logout')}</button>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                <button onClick={() => handleViewChange('report')} className="btn-gov-primary" style={{ width: '100%', justifyContent: 'center' }}>{t('nav.citizenLogin')}</button>
                <button onClick={() => handleViewChange('dept-auth')} className="btn-gov-secondary" style={{ width: '100%', justifyContent: 'center' }}>{t('nav.deptPortal')}</button>
              </div>
            )}
          </div>
        </div>
