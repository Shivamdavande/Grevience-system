...
function App() {
  ...
  const [isCitizenAuthenticated, setIsCitizenAuthenticated] = useState(!!localStorage.getItem('citizenAadhar'));
  const [userAadhar, setUserAadhar] = useState(localStorage.getItem('citizenAadhar') || '');
  ...
  const handleCitizenLogin = (aadhar) => {
    localStorage.setItem('citizenAadhar', aadhar);
    setIsCitizenAuthenticated(true);
    setUserAadhar(aadhar);
    setShowAuthModal(false);
    setView('report'); // or dashboard depending on flow, usually report is good
  };
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            {isDeptAuthenticated && (
              ...
              {isCitizenAuthenticated && (
                ...
                >
                  {t('nav.myDashboard') || 'User Dashboard'}
                </button>
              )}
              ...
              {(isDeptAuthenticated || isCitizenAuthenticated) ? (
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="px-5 py-2.5 bg-gov-saffron text-gov-navy rounded-xl font-bold text-xs uppercase tracking-widest hover:brightness-110 transition-all shadow-lg shadow-gov-saffron/20 flex items-center gap-2"
                  >
          ...
          {isMenuOpen && (
            ...
            >
              <div className="p-6 space-y-4">
                ...
                {isCitizenAuthenticated && (
                  <button onClick={() => handleViewChange('user-dashboard')} className="block w-full text-left py-2 font-bold text-gov-navy uppercase tracking-widest text-sm">User Dashboard</button>
                )}
                <div className="pt-4 border-t border-gray-100">
                  {(isDeptAuthenticated || isCitizenAuthenticated) ? (
                    ...
                    <>
                      <button onClick={() => { setIsMenuOpen(false); setShowAuthModal(true); }} className="w-full mb-3 py-4 bg-gov-saffron text-gov-navy rounded-xl font-bold uppercase tracking-widest text-xs">Citizen Login</button>
      ...
      <main className="flex-grow">
        <AnimatePresence mode="wait">
          {view === 'home' && (
            <HomePage
              key="home"
              onReportGrievance={() => isCitizenAuthenticated ? handleViewChange('report') : setShowAuthModal(true)}
              onTrackStatus={() => handleViewChange('track')}
              onCitizenLogin={() => isCitizenAuthenticated ? handleViewChange('report') : setShowAuthModal(true)}
          ...
          {view === 'report' && (
            <GrievanceForm
              ...
              onSuccess={() => handleViewChange('user-dashboard')}
          ...
          {view === 'user-dashboard' && <UserDashboardPage key="user-dashboard" userAadhar={userAadhar} onLogout={handleLogout} onNewGrievance={() => handleViewChange('report')} />}
        </AnimatePresence>
      </main>
